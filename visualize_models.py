...
def plot_model_architecture(model, index):
    ...
    layer_outputs = [getattr(layer, 'output_shape', 'N/A') for layer in model.layers]
...
def predict_and_visualize(models, data, labels):
    for i, model in enumerate(models):
        ...
        ax.set_title(f'Matriz de Confusión del Modelo {i + 1}')
        ax.set_xlabel('Predicted')
        ax.set_ylabel('True')
        plt.tight_layout()
        plot_file = os.path.join(output_dir, f"confusion_matrix_{i + 1}.png")
        plt.savefig(plot_file)
        plt.close()
...
dummy_data = np.random.rand(100, 224, 224, 3)  # Ejemplo de datos de entrada (imagen 224x224x3)
dummy_labels = np.random.randint(0, 10, 100)  # Ejemplo de etiquetas
